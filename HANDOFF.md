# HANDOFF — 2026-09-20 地下鉄動物園前80号 production反映後
...
- 最終app production SHA: `a783cf53d40a1aadeff426283e731580f8bcdcd4`（PR #207 merge）。同SHAで `Validate bus data` SUCCESS、GitHub Pages build/deploy SUCCESS を確認済み。
- live確認(PR #207時点): 公開 `sw.js` はv44、`timetable-extra.json` に地下鉄動物園前80号を確認。公開UIで地下鉄動物園前 → 80号 → 鶴町四丁目方面を選択し、休日2026-09-20 10:00基準で `10:01 / 10:26 / 10:46`、準備中非表示を確認済み。
- 西船町70号はproduction時刻表を表示。同停留所の70急行はEvidence未確認のため準備中を維持。
- Service Worker: **v44**。
- final regression(Issue #206時点): `npm run validate` PASS / smoke **40/40 PASS** / coverage audit **15/15 PASS**。
