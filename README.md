...
- **系統の行き先(方面)・時刻表は一部を実データで入力済みです。** P11データには
  ...
  **鶴町二丁目 80号(あべの橋方面)**、**幸町一丁目（西）29号・71号・87号・60号**、
  **鶴町一丁目 71号(なんば方面)・55号(大阪駅前方面)** の計7系統×方面を収録済みです。
  それ以外は架空の行き先・時刻を表示せず、「🚧 時刻表データ準備中」と明示します。
  平日/土曜/休日ダイヤの区別、深夜0時をまたぐ便にも対応しています。
...
- 停留所名・緯度経度・系統番号は実データ(停留所992件・系統1915件)ですが、
  方面・行先・時刻表は現在7系統×方面のみ収録済みで、その他は順次手動整備中です。
