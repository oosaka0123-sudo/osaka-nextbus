...
- **系統の行き先(方面)・時刻表はまだ入力されていません。** P11データには
  系統番号(例:「27号」)はあっても行き先の情報が無いため、これらは
  `data/timetable.json` に手動で登録する運用です(未入力の間は、
  架空の行き先・時刻を本物のように表示せず「🚧 時刻表データ準備中」と
  明示します)。方面(行き先)は同じ系統番号でも複数存在しうるほか、
  平日/土曜/休日ダイヤの区別、深夜0時をまたぐ便にも対応した構造に
  なっています。今後、現地の時刻表掲示など正当な手段で確認した内容を
  手動入力していく予定です。
...
10. 画面上部に「最終更新 2026/08/29」のようにデータの最終更新日を表示(`data/metadata.json` 由来)
...
- 停留所名・緯度経度・系統番号は実データ(停留所992件・系統1915件)ですが、
  系統の行き先(方面)・時刻表はまだ入力されていません(`data/timetable.json` が空)。
