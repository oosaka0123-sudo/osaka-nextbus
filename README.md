...
- **系統の行き先(方面)・時刻表はまだ分かっていません。** P11データには系統番号
  (例:「27号」)はあっても行き先の情報が無いため、方面プルダウンには
  系統番号をそのまま表示しています。正式な時刻表データが無い間は、
  架空の時刻を本物のように表示せず「🚧 時刻表データ準備中」と明示します。
  今後、現地の時刻表掲示など正当な手段で確認した内容を手動入力していく予定です。
...
- 停留所名・緯度経度・系統番号は実データ(停留所992件・系統1915件)ですが、
  系統の行き先(方面)・時刻表はまだ分かっていません(P11データに含まれないため)。
