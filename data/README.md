...
系統番号(例: `"27号"`)をそのまま使っており、行き先(例:「なんば行」)は
まだ分かりません。`timetable.json` もまだ空 `[]` のため、次のバス表示は
...
- `label`: プルダウンに表示する文字列(例: `"なんば方面"` または系統番号 `"27号"`)。
- `destination`: 次のバス表示欄に大きく表示する行き先(例: `"なんば行"`)。
  **行き先が分からない場合は、系統番号など判明している情報をそのまま
  入れること。架空の行き先を作ってはいけない。**
  (現在の実データは国土数値情報由来の系統番号のみで、行き先は未確定)
