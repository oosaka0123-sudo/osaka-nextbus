...
アプリ上では `js/timetable-loader.js` が2つの時刻表JSONを結合し、同一キーのextraを優先するため、2026-09-12時点で **合計42系統×方面**として扱われます。
...
2026-09-12時点の結合後収録数は **42系統×方面** です。`timetable.json` は8エントリ、`timetable-extra.json` は35エントリで、そのうち1エントリがbaseを意図的に上書きするため、結合後は42になります。
