...
└── timetable-extra.json  追加・補正時刻表 7系統×方面（上書き補正1件含む）
```

アプリ上では `js/timetable-loader.js` が2つの時刻表JSONを結合するため、**合計14系統×方面**として扱われます。
...
- なんば 71号 → 鶴町四丁目（平日のみ確認済み。土曜・休日は確認未完了）
- なんば 87号 → 新千歳経由・鶴町四丁目（平日のみ確認済み。土曜・休日は確認未完了）

91急行は土曜・休日運休です。なんば71号・87号は平日ダイヤのみデータ確認済みです（土曜・休日ダイヤは未入力・検証未完了）。
